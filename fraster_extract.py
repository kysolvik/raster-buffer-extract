...
def transform_x_to_lon(center_lat, buf_x):
    latlen = 111319.5
    lonlen = np.cos(center_lat*np.pi/180) * latlen
    buf_x = buf_x/lonlen
...
def get_all_cells(target_pts, sample_pts, ds, to_latlon=True):
    # This is the slowest section by far. Need to find a speed up
    if to_latlon:
        x = np.array(
                [transform_x_to_lon(sample_pts[0], target_pts.y[i]) + target_pts.x[i]
                    for i in range(target_pts.shape[0])]
                ).flatten()
        sample_pts[1] = transform_y_to_lat(sample_pts[1])
...
def full_pt_calc(target_pt, band, ds, sample_pts, stat='mean_max'):
    cells = get_all_cells(target_pt, sample_pts, ds)
    if stat=='mean_max':
        get_all_mean_max(band, cells, sample_pts.shape[1])
    elif stat=='all':
...
def random_buffer(gdf_coords, ds, radius=20, n_sample=1000, stat='mean_max'):
    sample_pts = generate_points(radius, n_sample)
    band = ds.read(1)
    return full_pt_calc(gdf_coords, band, ds, sample_pts, stat=stat)
